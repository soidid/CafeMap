...
function testAPI() {
  ...
  FB.api('/me', function (response) {
    ...
    document.getElementById('status').innerHTML = 'Glad to see you, ' + response.name + '!';

    console.log(response);

    if (USER_LOCATION === false) {
      getUserLocation().then(function (updateGeo) {
        if (updateGeo === true) {
          getLike(response.id);
        }
      });
...
function getLikeCont(page_id) {
  FB.api(page_id, function (response) {
    if (response && !response.error) {
      ...
      handleLikeCallback(response).then(function () {
...
function handleLikeCallback(response) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    //First get geo ones
    getGeoList(response.data).then(function (geoList) {
      ...
      //Then get distance
      var count = 0;
      geoList.map(function (item, i) {
        getShopDistance(item).then(function (item) {
...
function getGeoList(data) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    data.map(function (item, i) {
      getLocation(item.id).then(function (place_info) {
...
function renderResult() {
  ...
  //sort
  USER_LIKE.map(function (item, i) {
    if (!item.distance) {
      USER_LIKE.splice(i, 1);
    }
  });
  USER_LIKE.sort(function (a, b) {
...
function getLocation(page_id) {
  return new Promise(function (resolve, reject) {
    FB.api('/' + page_id, { fields: 'name,id,location,category' }, function (response) {
      if (response && !response.error) {
        resolve(response);
      } else {
        resolve(false);
